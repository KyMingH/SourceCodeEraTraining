  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="转出班级" prop="fromClass">
          <el-select v-model="queryParams.fromClass" placeholder="转出班级">
            <el-option
        ...
        <el-form-item label="转入班级" prop="toClass">
          <el-select v-model="queryParams.toClass" placeholder="转入班级">
            <el-option
              v-for="class_ in classList"
              :key="class_.id"
              :label="class_.className"
    ...
    <el-table v-loading="loading" :data="recordsList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="转出班级" align="center" prop="fromClass" />
      <el-table-column label="转入班级" align="center" prop="toClass" />
      <el-table-column label="转班时间" align="center" prop="transferDate" width="180">
        <template slot-scope="scope">
          <span>{{ parseTime(scope.row.transferDate, '{y}-{m}-{d}') }}</span>
        </template>
      </el-table-column>
      <el-table-column label="转班原因" align="center" prop="reason" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="转出班级" prop="fromClass">
          <el-select v-model="form.fromClass" placeholder="转出班级">
            <el-option
              v-for="class_ in classList"
              :key="class_.id"
              :label="class_.className"
        ...
        <el-form-item label="转入班级" prop="toClass">
          <el-select v-model="form.toClass" placeholder="转入班级">
            <el-option
              v-for="class_ in classList"
              :key="class_.id"
              :label="class_.className"
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除记录转班记录编号为"' + ids + '"的数据项？').then(function() {
