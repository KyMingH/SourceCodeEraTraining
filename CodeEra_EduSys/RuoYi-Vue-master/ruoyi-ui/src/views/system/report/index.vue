...
export default {
  ...
  methods: {
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加class_daily_report";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      ...
      getReport(reportId).then(response => {
        ...
        this.open = true;
        this.title = "修改class_daily_report";
      });
    },
    /** 提交按钮 */
    submitForm() {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除class_daily_report编号为"' + reportIds + '"的数据项？').then(function() {
