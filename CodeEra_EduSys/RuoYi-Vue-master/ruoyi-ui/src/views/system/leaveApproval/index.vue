<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['system:leaveApproval:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['system:leaveApproval:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:leaveApproval:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:leaveApproval:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="leaveApprovalList" @selection-change="handleSelectionChange">
    ...
    <!-- 添加或修改LeaveApproval对话框 -->
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="请假ID" prop="leaveId">
          <el-input v-model="form.leaveId" placeholder="请输入请假ID" />
        </el-form-item>
        <el-form-item label="审批人" prop="approver">
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        leaveId: [
          { required: true, message: "请假ID不能为空", trigger: "blur" }
        ],
        approvalStatus: [
  ...
  methods: {
    ...
    handleApproval(row) {
      ...
      getLeaveApproval(approvalId).then(response => {
    ...
    getList() {
      this.loading = true;
      listLeaveApproval(this.queryParams).then(response => {
        this.leaveApprovalList = response.rows;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.approvalId != null) {
            updateLeaveApproval(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addLeaveApproval(this.form).then(response => {
    ...
    submitForm_approval() {
      this.$refs["form_approval"].validate(valid => {
        if (valid) {
            getLeave(this.form_approval.leaveId).then(response => {
              if (response && response.data) {
                this.form_leave = response.data;
                if (this.form_approval.approvalStatus == '01')
                {
                  this.form_leave.approvalStatus = '通过'
                  return updateLeave(this.form_leave); // 使用 return 以便于链式调用
                }
                else if(this.form_approval.approvalStatus == '02')
                {
                  this.form_leave.approvalStatus = '驳回'
                  return updateLeave(this.form_leave); // 使用 return 以便于链式调用
                }
              }
            })
            ...
            .catch(error => {
              ...
              this.$modal.msgError("更新状态失败"); // 假设这是显示错误消息的方法
            });
            updateLeaveApproval(this.form_approval).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除LeaveApproval编号为"' + approvalIds + '"的数据项？').then(function() {
        return delLeaveApproval(approvalIds);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/leaveApproval/export', {
        ...this.queryParams
      }, `leaveApproval_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
