<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="managementList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作ID" align="center" prop="operationId" />
      ...
      <el-table-column label="操作日期" align="center" prop="operationDate" width="180">
        <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加class_management";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const operationId = row.operationId || this.ids
      getManagement(operationId).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改class_management";
      });
    },
    /** 提交按钮 */
    submitForm() {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除class_management编号为"' + operationIds + '"的数据项？').then(function() {
        return delManagement(operationIds);
      }).then(() => {
        this.getList();
