<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="请假开始日期" prop="leaveStartDate">
        <el-date-picker clearable
          v-model="queryParams.leaveStartDate"
          type="date"
      ...
      <el-form-item label="请假结束日期" prop="leaveEndDate">
        <el-date-picker clearable
          v-model="queryParams.leaveEndDate"
          type="date"
          value-format="yyyy-MM-dd"
    ...
    <el-table v-loading="loading" :data="leaveList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="请假开始日期" align="center" prop="leaveStartDate" width="180">
        <template slot-scope="scope">
          <span>{{ parseTime(scope.row.leaveStartDate, '{y}-{m}-{d}') }}</span>
        </template>
      </el-table-column>
      <el-table-column label="请假结束日期" align="center" prop="leaveEndDate" width="180">
        <template slot-scope="scope">
          <span>{{ parseTime(scope.row.leaveEndDate, '{y}-{m}-{d}') }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="请假类型">
          <editor v-model="form.leaveType" :min-height="192"/>
        </el-form-item>
        <el-form-item label="请假开始日期" prop="leaveStartDate">
          <el-date-picker clearable
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除student_leave编号为"' + leaveIds + '"的数据项？').then(function() {
