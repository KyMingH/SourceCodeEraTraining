<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="studentList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="沟通内容" align="center" prop="communicationContent" />
      <el-table-column label="沟通日期" align="center" prop="communicationDate" width="180">
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="跟进人" prop="followUpPerson">
          <el-input v-model="form.followUpPerson" placeholder="请输入跟进人" />
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加problem_student";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const recordId = row.recordId || this.ids
      getStudent(recordId).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改problem_student";
      });
    },
    /** 提交按钮 */
    submitForm() {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除problem_student编号为"' + recordIds + '"的数据项？').then(function() {
        return delStudent(recordIds);
      }).then(() => {
        this.getList();
