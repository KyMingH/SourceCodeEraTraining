<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['system:studentProcessApproval:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['system:studentProcessApproval:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:studentProcessApproval:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:studentProcessApproval:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="studentProcessApprovalList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="审批类型" align="center" prop="processType">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.application_type" :value="scope.row.processType"/>
        </template>
      </el-table-column>
    ...
    <!-- 添加或修改StudentProcessApproval对话框 -->
    ...
    <el-dialog :title="title" :visible.sync="open_approval" width="500px" append-to-body>
      <el-form ref="form_approval" :model="form_approval" :rules="rules_approval" label-width="80px">
        ...
        <el-form-item label="审批类型" prop="processType">
          <el-select v-model="form_approval.processType" placeholder="请选择审批类型">
            <el-option
              v-for="dict in dict.type.application_type"
              :key="dict.value"
              :label="dict.label"
...
export default {
  name: "StudentProcessApproval",
  dicts: ['approval_status2', 'application_type'],
  data() {
    return {
      // 遮罩层
      loading: true,
      ...
      // 总条数
      total: 0,
      // StudentProcessApproval表格数据
      studentProcessApprovalList: [],
      // 弹出层标题
      title: "",
      // 是否显示弹出层
      open: false,
  ...
  methods: {
    ...
    handleApproval(row) {
      ...
      getStudentProcessApproval(approvalId).then(response => {
    ...
    getList() {
      this.loading = true;
      listStudentProcessApproval(this.queryParams).then(response => {
        this.studentProcessApprovalList = response.rows;
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加StudentProcessApproval";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      ...
      getStudentProcessApproval(approvalId).then(response => {
        ...
        this.open = true;
        this.title = "修改StudentProcessApproval";
      });
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.approvalId != null) {
            updateStudentProcessApproval(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addStudentProcessApproval(this.form).then(response => {
    ...
    submitForm_approval() {
      ...
      this.$refs["form_approval"].validate(valid => {
        if (valid) {
          // 提交审批
          updateStudentProcessApproval(this.form_approval).then(response => {
          ...
          if (this.form_approval.approvalStatus === '01') {
            getStudents(this.form_approval.studentId).then(response => {
              if (response && response.data) {
                this.form_stu = response.data; // 正确获取并赋值学生信息
                // 根据需要更新 form_stu 对象的属性
                console.log(this.form_approval.processType)
                this.form_stu.status = this.form_approval.processType;

                return updateStudents(this.form_stu); // 更新学生信息
              }
            })
            .then(response => {
              // 更新学生信息成功后的操作
              this.$modal.msgSuccess("学生信息更新成功");
            })
            .catch(error => {
              console.error("更新学生信息失败：", error);
              this.$modal.msgError("更新学生信息失败"); // 显示错误消息
            });
          }
        }
      });
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除StudentProcessApproval编号为"' + approvalIds + '"的数据项？').then(function() {
        return delStudentProcessApproval(approvalIds);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/studentProcessApproval/export', {
        ...this.queryParams
      }, `studentProcessApproval_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
