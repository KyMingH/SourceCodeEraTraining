...
export default {
  name: "Application",
  dicts: ['approval_status', 'application_type'],
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加student_application";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      ...
      getApplication(applicationId).then(response => {
        ...
        this.open = true;
        this.title = "修改student_application";
      });
    },
    /** 提交按钮 */
    submitForm() {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除student_application编号为"' + applicationIds + '"的数据项？').then(function() {
